...
<script>
    document.addEventListener('DOMContentLoaded', (event) => {
        ...
        // VARIABLE DE CONTROL: Determina si la lectura automática está activa
        let autoRead = true; 
        ...
        const readIncrementalContent = (element) => {
            // Si el modo autoRead está desactivado, no hace nada
            if (!autoRead) return;
        ...
        const readCurrentSlide = () => {
            // Si el modo autoRead está desactivado, no lee al cambiar de diapositiva
            if (!autoRead) return;

            if (utterance && synth.speaking) synth.cancel();
            
            // Usamos Reveal.getCurrentSlide() para evitar leer el H1 de la sección padre incorrectamente
        ...
        playButton.addEventListener('click', () => {
            if (synth.speaking) {
                ...
                // Al presionar reproducir, REACTIVAMOS la lectura automática
                autoRead = true; 
                readCurrentSlide();
        ...
        // --- Botón de Silencio (Mute Global) ---
        const muteButton = document.createElement('button');
        muteButton.textContent = '🔇'; 
        muteButton.title = "Silenciar totalmente"; 
        ...
        muteButton.addEventListener('click', () => {
            // AJUSTE: Desactiva la lectura automática y detiene el audio actual
            autoRead = false; 
            synth.cancel(); 
            playButton.textContent = 'Reproducir'; 
        });
